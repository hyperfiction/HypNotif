...
{
{
	HX_SOURCE_POS("/Volumes/workspaceHaxe/inthebox/src/org/shoebox/utils/system/Signal1.hx",47)
	super::__construct();
}
;
	return null();
...
Void Signal1_obj::emit( Dynamic arg){
{
		...
		HX_SOURCE_POS("/Volumes/workspaceHaxe/inthebox/src/org/shoebox/utils/system/Signal1.hx",59)
		for(::cpp::FastIterator_obj< Dynamic > *__it = ::cpp::CreateFastIterator< Dynamic >(this->_oQueue->iterator());  __it->hasNext(); ){
			Dynamic l = __it->next();
			{
				HX_SOURCE_POS("/Volumes/workspaceHaxe/inthebox/src/org/shoebox/utils/system/Signal1.hx",61)
				l->__Field(HX_CSTRING("listener"),true)(arg);
				HX_SOURCE_POS("/Volumes/workspaceHaxe/inthebox/src/org/shoebox/utils/system/Signal1.hx",62)
				this->_check(l);
			}
;
		}
